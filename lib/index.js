/**
 * dsh-inherit — host half.
 *
 * 扫描各智能体来源（Claude Code / Cursor / Codex / cc-switch），提供
 * /api/dsh-inherit/sources（盘点）与 /api/dsh-inherit/run（执行导入）两个
 * 路由给浏览器半区。导入逻辑（lib/import.js）是纯函数，无 DSH 依赖。
 * 所有路由带 loopback-only 信任护栏（读用户文件、写配置，不能暴露到局域网）。
 */
import { detectSources, runImport, targetOptions, KNOWN_TOOL_ACTIONS } from './import.js'
import Schema from '@deepseek-ai/schemastery'

export const name = 'inherit'

/** webServer 挂 /api 路由；systemPrompt 向模型公告插件存在。 */
export const inject = ['webServer', 'systemPrompt']

/**
 * 插件配置。在 cordis.patch.yml 的 `- id: ui-inherit` 行下写 config 覆盖，
 * 例如：
 *   config:
 *     profile: web
 *     target: patch
...
const GUIDANCE =
  '本机已安装 dsh-inherit 插件（导入智能体设置）：设置 → 通用设置 → 「导入智能体设置」。' +
...
/** 从 ToolExecutionResult 提取可读文本（content 块的 text 拼接；兜底 JSON 截断）。 */
function toolResultText(result) {
  const blocks = result?.content
  if (Array.isArray(blocks)) {
    const text = blocks.map((b) => (b && typeof b.text === 'string' ? b.text : '')).filter(Boolean).join('\n')
    if (text) return text
  }
  try { return JSON.stringify(result)?.slice(0, 500) ?? '' } catch { return '' }
}

/** 程序化调用委派工具。失败不抛——导入报告要带上失败原因继续展示。
 *  入参用 ToolExecutionInput：callId 是品牌字符串（运行期就是普通串），参数字段名是 arguments。 */
async function invokeDelegate(toolsSvc, toolName, action, dryRun) {
  if (!toolsSvc?.execute) return { tool: toolName, ok: false, output: 'tools 服务不可用' }
  try {
    const result = await toolsSvc.execute({
      callId: `inherit:${Date.now()}`,
      name: toolName,
      arguments: action.args(dryRun),
      signal: AbortSignal.timeout(60_000),
    })
    return { tool: toolName, ok: result?.isError === false, output: toolResultText(result) }
  } catch (e) { return { tool: toolName, ok: false, output: String(e) } }
}
    ...
    },
    {
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          // 能力委派：勾选类别带 delegate 且工具在已知规则表内 → 摘出来程序化调用，
          // 不走 runImport（同一工具被多个来源委派时只调一次）
          const snapshot = detectSources(collectEvidence(ctx), pluginOpts)
          const delegateTools = new Map()
          const runSelections = selections.map((sel) => {
            const cats = snapshot.find((s) => s.id === sel.source)?.categories ?? {}
            const keep = []
            for (const c of Array.isArray(sel.categories) ? sel.categories : []) {
              const action = cats[c]?.delegate ? KNOWN_TOOL_ACTIONS[cats[c].delegate] : null
              if (action) delegateTools.set(cats[c].delegate, action)
              else keep.push(c)
            }
            return { ...sel, categories: keep }
          }).filter((sel) => sel.categories.length)
          const report = await runImport(runSelections, {
            ...pluginOpts,
            // 请求级 target 优先，未带则用 Config 默认值
            target: body.target ?? config.target,
            dryRun,
          })
          if (delegateTools.size) {
            const toolsSvc = ctx.get('tools')
            report.delegated = []
            for (const [toolName, action] of delegateTools) {
              report.delegated.push(await invokeDelegate(toolsSvc, toolName, action, dryRun))
            }
          }
