...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function ImportModal(props) {
      ...
      /** 渲染导入报告；isPreview=true 时是 dry-run 预演结果，标题与标注区分。 */
      function renderReport(isPreview) {
        return React.createElement('div', { className: 'dsh-import-report' },
          React.createElement('strong', null, isPreview ? '预演结果' : '导入完成'),
          React.createElement('div', null, 'MCP ' + (isPreview ? '将导入' : '已导入') + '：' + (report.mcp.imported.length ? report.mcp.imported.join('、') : '无')),
          report.mcp.skipped.length > 0 && React.createElement('div', null, 'MCP 跳过：' + report.mcp.skipped.join('；')),
          React.createElement('div', null, 'Skills ' + (isPreview ? '将导入' : '已导入') + '：' + (report.skills.imported.length ? report.skills.imported.join('、') : '无')),
