...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    // 主题适配：用 DSH 的 --dsw-alias-* 语义变量（亮/暗自动切换），文字继承主题色；
    // 次要文字/边框/按钮用中性灰透明，亮暗两模式都清晰。
    var __css =
      '.dsh-import-overlay{position:fixed;inset:0;background:rgba(0,0,0,.45);backdrop-filter:blur(3px);z-index:999;display:flex;align-items:center;justify-content:center}' +
      '.dsh-import-card{background:var(--dsw-alias-bg-overlay,#1f2330);color:inherit;border:1px solid var(--dsw-alias-border-inverted,transparent);border-radius:14px;width:min(640px,92vw);max-height:82vh;display:flex;flex-direction:column;box-shadow:0 18px 60px rgba(0,0,0,.5)}' +
      '.dsh-import-head{padding:16px 20px 10px;border-bottom:1px solid var(--dsw-alias-border-l2,rgba(128,128,128,.25))}' +
      ...
      '.dsh-import-src{border:1px solid rgba(128,128,128,.28);border-radius:10px;margin:8px 0;overflow:hidden;transition:opacity .18s,border-color .18s}' +
      '.dsh-import-src.on{border-color:rgba(59,130,246,.55)}' +
      ...
      '.dsh-import-src-head:hover{background:rgba(128,128,128,.08)}' +
      '.dsh-import-src-body{padding:2px 14px 12px;border-top:1px dashed rgba(128,128,128,.25)}' +
      ...
      '.dsh-import-foot{display:flex;justify-content:flex-end;gap:10px;padding:12px 20px;border-top:1px solid var(--dsw-alias-border-l2,rgba(128,128,128,.25))}' +
      '.dsh-import-btn{border-radius:8px;border:1px solid rgba(128,128,128,.35);background:rgba(128,128,128,.12);color:inherit;padding:7px 16px;font-size:13px;cursor:pointer}' +
      '.dsh-import-btn-primary{background:#3b82f6;border-color:#3b82f6;color:#fff}' +
      '.dsh-import-btn:disabled{opacity:.5;cursor:not-allowed}' +
      '.dsh-import-report{margin:8px 0;font-size:13px;line-height:1.7}' +
      /* 开关 */
      '.dsh-import-switch{position:relative;width:38px;height:21px;flex:none;cursor:pointer;border:0;background:rgba(128,128,128,.3);border-radius:12px;transition:background .18s;padding:0}' +
      '.dsh-import-switch.on{background:#3b82f6}' +
      '.dsh-import-switch-knob{position:absolute;top:2px;left:2px;width:17px;height:17px;border-radius:50%;background:#fff;transition:transform .18s}' +
      ...
      return React.createElement('div', { className: 'dsh-import-overlay', onClick: function (e) { if (e.target === e.currentTarget) onClose() } },
        React.createElement('div', { className: 'dsh-import-card' },
          ...
          React.createElement('div', { className: 'dsh-import-body' },
            ...
            phase === 'error' && React.createElement('div', { className: 'dsh-import-report' }, '出错：' + error),
    ...
    function ImportRow() {
      ...
      return React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 10, padding: '10px 0', borderBottom: '1px solid var(--dsw-alias-border-l2, rgba(128,128,128,.25))' } },
        React.createElement('div', { style: { flex: 1 } },
          React.createElement('div', { style: { fontWeight: 600, fontSize: 13.5 } }, '导入智能体设置'),
          React.createElement('div', { style: { fontSize: 12, opacity: 0.6, marginTop: 2 } }, '从 Claude Code / Cursor / Codex / cc-switch 导入 MCP 与 Skills'),
        ),
