...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    var __css =
      ...
      '.dsh-import-stats{display:flex;gap:8px;margin:10px 0 2px}' +
      '.dsh-import-stat{flex:1;min-width:0;border:1px solid var(--dsw-alias-border-l2,rgba(255,255,255,.12));border-radius:10px;padding:9px 12px}' +
      '.dsh-import-stat-num{font-size:20px;font-weight:600;line-height:1.25}' +
      '.dsh-import-stat-num.ok{color:var(--dsw-alias-state-success-primary,rgb(34,197,94))}' +
      '.dsh-import-stat-num.zero{opacity:.35}' +
      '.dsh-import-stat-label{font-size:11.5px;opacity:.6;margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}' +
      '.dsh-import-sec{margin-top:14px}' +
      '.dsh-import-sec-title{font-size:12.5px;font-weight:600;opacity:.8;margin:0 0 8px}' +
      '.dsh-import-chips{display:flex;flex-wrap:wrap;gap:6px}' +
      '.dsh-import-chip{display:inline-flex;align-items:center;gap:5px;border:1px solid var(--dsw-alias-border-l2,rgba(255,255,255,.12));border-radius:6px;padding:2px 8px;font-size:12px;line-height:20px;max-width:100%}' +
      '.dsh-import-chip-check{color:var(--dsw-alias-state-success-primary,rgb(34,197,94));font-weight:600}' +
      '.dsh-import-chip-skip{opacity:.55;font-size:11.5px}' +
      '.dsh-import-skip{border:1px dashed var(--dsw-alias-border-l2,rgba(255,255,255,.12));border-radius:8px;margin:6px 0;overflow:hidden}' +
      '.dsh-import-skip-head{display:flex;align-items:center;gap:8px;width:100%;padding:7px 10px;background:transparent;border:0;color:inherit;font-size:12px;cursor:pointer;text-align:left}' +
      '.dsh-import-skip-head:hover{background:var(--dsw-alias-interactive-bg-hover,rgba(255,255,255,.08))}' +
      '.dsh-import-skip-count{margin-left:auto;opacity:.55;flex:none}' +
      '.dsh-import-skip-body{padding:2px 10px 9px;display:flex;flex-wrap:wrap;gap:6px}' +
      '.dsh-import-notes{margin-top:14px;padding-top:8px;border-top:1px dashed var(--dsw-alias-border-l2,rgba(255,255,255,.12))}' +
      /* 工具调用结果（能力委派） */
      '.dsh-import-tool{display:flex;align-items:center;gap:8px;font-size:12.5px;padding:4px 0}' +
      '.dsh-import-tool-out{white-space:pre-wrap;word-break:break-all;font-size:11.5px;opacity:.75;margin:2px 0 8px;padding:8px 10px;border:1px dashed var(--dsw-alias-border-l2,rgba(255,255,255,.12));border-radius:8px;max-height:160px;overflow:auto}' +
      /* 写入位置选择器 */
      '.dsh-import-target-row{display:flex;align-items:center;gap:10px;margin-top:12px;padding-top:10px;border-top:1px dashed var(--dsw-alias-border-l2,rgba(255,255,255,.12));font-size:12.5px}' +
      '.dsh-import-target-label{opacity:.65;flex:none}' +
      '.dsh-import-target-select{flex:1;min-width:0;background:var(--dsw-alias-bg-overlay,rgb(97,102,107));color:var(--dsw-alias-label-primary,rgb(249,250,251));border:1px solid var(--dsw-alias-border-l2,rgba(255,255,255,.12));border-radius:8px;padding:6px 8px;font-size:12.5px;cursor:pointer}' +
    ...
    function SkipGroup(props) {
      ...
      return React.createElement('div', { className: 'dsh-import-skip' },
        React.createElement('button', {
          type: 'button', className: 'dsh-import-skip-head',
          onClick: function () { setOpen(!open) },
        },
          React.createElement('span', null, open ? '▾' : '▸'),
          React.createElement('span', null, props.reason),
          React.createElement('span', { className: 'dsh-import-skip-count' }, props.names.length + ' 项'),
        ),
        open && React.createElement('div', { className: 'dsh-import-skip-body' },
          props.names.map(function (n, i) {
            return React.createElement('span', { key: i, className: 'dsh-import-chip dsh-import-chip-skip' }, n)
          }),
        ),
      )
    }
    ...
    function ImportModal(props) {
      ...
      var backBtn = React.createElement('button', { className: 'dsh-import-btn', onClick: function () { setPhase('ready') } }, '返回')

      /** 渲染导入报告；isPreview=true 时是 dry-run 预演结果，标题与措辞区分。 */
      function renderReport(isPreview) {
        var verb = isPreview ? '将导入' : '已导入'
        return React.createElement('div', { className: 'dsh-import-report' },
          React.createElement('strong', null, isPreview ? '预演结果（未实际写入）' : '导入完成'),
          React.createElement('div', { className: 'dsh-import-stats' },
          ...
          report.delegated && report.delegated.length > 0 && React.createElement('div', { className: 'dsh-import-sec' },
            React.createElement('p', { className: 'dsh-import-sec-title' }, isPreview ? '工具调用（预演）' : '工具调用'),
            report.delegated.map(function (d, i) {
              return React.createElement('div', { key: i },
                React.createElement('div', { className: 'dsh-import-tool' },
                  React.createElement('span', {
                    className: 'dsh-import-chip-check',
                    style: d.ok ? undefined : { color: 'var(--dsw-alias-state-error-primary, rgb(242,90,90))' },
                  }, d.ok ? '✓' : '✗'),
                  React.createElement('span', null, d.tool),
                ),
                d.output && React.createElement('div', { className: 'dsh-import-tool-out' }, d.output),
              )
            }),
          ),
          report.notes.length > 0 && React.createElement('div', { className: 'dsh-import-notes' },
      ...
      function statCard(num, label, ok) {
        return React.createElement('div', { className: 'dsh-import-stat' },
          React.createElement('div', { className: 'dsh-import-stat-num' + (ok ? ' ok' : num === 0 ? ' zero' : '') }, String(num)),
          React.createElement('div', { className: 'dsh-import-stat-label' }, label),
        )
      }
      ...
      function reportSection(title, imported, skipped) {
        ...
        return React.createElement('div', { className: 'dsh-import-sec' },
          React.createElement('p', { className: 'dsh-import-sec-title' }, title),
          imported.length > 0 && React.createElement('div', { className: 'dsh-import-chips' },
            imported.map(function (n, i) {
              return React.createElement('span', { key: i, className: 'dsh-import-chip' },
                React.createElement('span', { className: 'dsh-import-chip-check' }, '✓'), n)
            }),
          ),
      ...
      return React.createElement('div', { className: 'dsh-import-overlay', onClick: function (e) { if (e.target === e.currentTarget) onClose() } },
        React.createElement('div', { className: 'dsh-import-card' },
          React.createElement('div', { className: 'dsh-import-head' },
            ...
            PHASE_HINTS[phase] && React.createElement('p', { className: 'dsh-import-sub' }, PHASE_HINTS[phase]),
          ),
          React.createElement('div', { className: 'dsh-import-body' },
            ...
            phase === 'ready' && targets.length > 1 && React.createElement('div', { className: 'dsh-import-target-row' },
              React.createElement('span', { className: 'dsh-import-target-label' }, 'MCP 写入位置'),
              React.createElement('select', {
                className: 'dsh-import-target-select',
                value: target,
          ...
          React.createElement('div', { className: 'dsh-import-foot' },
            phase === 'ready' && React.createElement('button', { className: 'dsh-import-btn', onClick: onClose }, '取消'),
