...
const CAPABILITY_PATTERNS = {
  ...
  providers: {
    tool: /ccswitch|(?=.*provider)(?=.*(sync|rout|switch))/i,
  ...
  prompts: {
    tool: /(?=.*prompt)(?=.*(import|sync|migrat|move))/i,
...
/** 能力提示小字：不带类别名前缀（小字本来就显示在该类别行内，前缀是复读）。 */
function capabilityNote(capability, evidence) {
  const hit = capabilityMatch(evidence, capability)
  if (hit?.kind === 'tool') return `由已注册工具「${hit.name}」处理`
  if (hit?.kind === 'guess') return `检测到可能相关的插件「${hit.name}」，未确认`
  return `未检测到处理该能力的插件，本插件不导入此项`
}
...
export function detectSources(evidence = [], opts = {}) {
  ...
  const sources = [
    {
      ...
      categories: {
        ...
        memory: cat(false, false, countMemoryFiles(claudeRoot), capabilityNote('memory', evidence)),
        sessions: cat(false, false, 0, capabilityNote('sessions', evidence)),
      },
    },
    {
      ...
      categories: {
        ...
        memory: cat(false, false, 0, capabilityNote('memory', evidence)),
        sessions: cat(false, false, 0, capabilityNote('sessions', evidence)),
      },
    },
    {
    ...
    },
    {
      ...
      categories: {
        providers: cat(false, false, 0, capabilityNote('providers', evidence)),
        mcp: cat(true, false, 0),
        skills: cat(true, countSkillDirs(join(ccRoot, 'skills')) > 0, countSkillDirs(join(ccRoot, 'skills'))),
        prompts: cat(false, false, 0, capabilityNote('prompts', evidence)),
        memory: cat(false, false, 0, capabilityNote('memory', evidence)),
        sessions: cat(false, false, 0, capabilityNote('sessions', evidence)),
      },
    },
  ...
  if (db && !db.error) {
    ...
    cc.categories.providers = cat(false, false, (db.providers ?? []).length, capabilityNote('providers', evidence))
    cc.categories.prompts = cat(false, false, (db.prompts ?? []).length, capabilityNote('prompts', evidence))
  }
  return sources
}
 ...
 *  mcp.json 必须在 ~/.dsh 下才会被读——所以选项是固定两个合法位置 + 自动。 */
export function targetOptions(profile = 'web') {
  return [
    { id: 'auto', label: '自动选择', path: '' },
    { id: 'patch', label: '组合层（cordis.patch.yml）', path: profilePatchPath(profile) },
    { id: 'mcp-json', label: 'mcp.json（zebbkira 版 UI）', path: join(DSH_HOME, 'mcp.json') },
  ]
}
...
export async function importMcpToPatch(servers, { profile = 'web', dryRun = false } = {}) {
  ...
  const block = `\n# --- dsh-import-agent-settings 导入 ${stamp} ---\n` + stringifyYaml([layer])
  if (!dryRun) {
    backupRolling(path)
