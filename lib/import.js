...
export function detectSources(loadedNames = []) {
  ...
  if (db && !db.error) {
    ...
    cc.categories.providers = cat(true, (db.providers ?? []).length > 0, (db.providers ?? []).length, capabilityNote('providers', loadedNames, '模型路由：'))
    cc.categories.prompts = cat(true, (db.prompts ?? []).length > 0, (db.prompts ?? []).length, '暂不支持')
  }
  return sources
}
