...
const CAPABILITY_PATTERNS = {
  ...
  // 会话导入需显式含 import/move/sync 语义，避免命中 dsh 核心的会话管理插件
  sessions: /claude[._-]?move|chat[._-]?import|session[._-]?(import|move|sync)/i,
}
...
function capabilityNote(capability, loadedNames, what) {
  ...
  if (hit) return `${what}由已加载插件「${hit.replace(/^include:/, '')}」处理`
...
export function detectSources(loadedNames = [], opts = {}) {
...
export function runImport(selections, opts = {}) {
  ...
  if (allServers.length) {
    ...
    if (resolvedTarget === 'patch') {
      const r = importMcpToPatch(allServers, { dryRun, profile: opts.profile ?? 'web' })
