...
function capabilityNote(capability, evidence, what) {
  const hit = capabilityMatch(evidence, capability)
  if (hit?.kind === 'tool') return `${what}由已注册工具「${hit.name}」处理`
  if (hit?.kind === 'guess') return `${what}检测到可能相关的插件「${hit.name}」，未确认`
...
export function detectSources(evidence = [], opts = {}) {
  ...
  const sources = [
    {
      ...
      categories: {
        ...
        memory: cat(false, false, countMemoryFiles(claudeRoot), capabilityNote('memory', evidence, '记忆：')),
        sessions: cat(false, false, 0, capabilityNote('sessions', evidence, '会话：')),
      },
    },
    {
    ...
    },
    {
      ...
      categories: {
        providers: cat(false, false, 0, capabilityNote('providers', evidence, '模型路由：')),
  ...
  if (db && !db.error) {
    ...
    cc.categories.providers = cat(false, false, (db.providers ?? []).length, capabilityNote('providers', evidence, '模型路由：'))
